...
symptoms1 = ['itching','skin_rash','nodal_skin_eruptions','continuous_sneezing','shivering','chills','joint_pain','stomach_pain','acidity',
    ...
    'swelling_of_stomach','swelled_lymph_nodes','malaise','blurred_and_distorted_vision','phlegm','throat_irritation','redness_of_eyes','sinus_pressure',
    ...
    'irritation_in_anus','neck_pain','dizziness','cramps','bruising','obesity','swollen_legs','swollen_blood_vessels','puffy_face_and_eyes','enlarged_thyroid',
...
diagnosis = ['Fungal infection','Allergy','GERD','Chronic cholestasis','Drug Reaction','Peptic ulcer diseae','AIDS','Diabetes','Gastroenteritis',
    ...
    'Dimorphic hemmorhoids(piles)','Heartattack','Varicoseveins','Hypothyroidism','Hyperthyroidism','Hypoglycemia','Osteoarthristis','Arthritis',
...
def NaiveBayes(s1='None', s2='None', s3='None', s4='None', s5='None'):
    ...
    userSymptoms = [s1, s2, s3, s4, s5]
    print(userSymptoms)

    for k in range(0, len(symptoms1)):
        for z in userSymptoms:
            if(z == symptoms1[k]):
                symptoms2[k] = 1
    ...
    h='no'
    for a in range(0, len(diagnosis)):
        if(diagnosis[predicted] == diagnosis[a]):
            h='yes'
            break
